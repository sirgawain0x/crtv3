import { getFilterFromBlock, getFilterToBlock, isAddressFactory, } from '../runtime/filter.js';
import { hexToBytes, hexToNumber, keccak256 } from "viem";
 ...
 */
export function isFilterInBloom({ block, filter, }) {
    // Return `false` for out of range blocks
    if (hexToNumber(block.number) < getFilterFromBlock(filter) ||
    ...
    if (filter.address === undefined)
        ...
        if ((filter.address.address === undefined ||
            (Array.isArray(filter.address.address)
                ? filter.address.address.some((address) => isInBloom(block.logsBloom, address))
        ...
        if (filter.address.length === 0) {
            ...
            isAddressInBloom = filter.address.some((address) => isInBloom(block.logsBloom, address));
        }
    }
    else {
        // single address case
