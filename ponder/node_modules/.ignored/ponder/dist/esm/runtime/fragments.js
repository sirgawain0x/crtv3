...
export const getFactoryFragments = (factory) => {
    const fragments = [];
    ...
    if (isAddressFactory(address)) {
        for (const fragmentAddress of Array.isArray(address.address)
            ...
            fragments.push({
                fragment,
                adjacentIds: [
                    `${fragmentAddress ?? null}_${address.eventSelector}_${address.childAddressLocation}`,
                ],
            });
        }
    }
            ...
            for (const fromAddressAdjacentId of fromAddressFragment.adjacentIds) {
                for (const toAddressAdjacentId of toAddressFragment.adjacentIds) {
                    for (const adjacentTxr of filter.hasTransactionReceipt
                        ? [1]
                        : [0, 1]) {
                        adjacentIds.push(`trace_${chainId}_${fromAddressAdjacentId}_${toAddressAdjacentId}_${functionSelector}_${adjacentTxr}`);
                    }
                }
            }
            fragments.push({ fragment, adjacentIds });
                            ...
                            for (const adjacentTopic2 of fragmentTopic2
                                ...
                                for (const adjacentTopic3 of fragmentTopic3
                                    ...
                                    for (const adjacentTxr of filter.hasTransactionReceipt
                                        ? [1]
                                        : [0, 1]) {
                                        adjacentIds.push(`log_${chainId}_${addressAdjacentId}_${eventSelector}_${adjacentTopic1}_${adjacentTopic2}_${adjacentTopic3}_${adjacentTxr}`);
                                    }
                                }
                            }
                        }
...
export const decodeFragment = (fragmentId) => {
    ...
    switch (type) {
        ...
        case "factory_log": {
            const [, chainId, address, eventSelector, childAddressLocation, fromBlock, toBlock,] = fragmentId.split("_");
            return {
                type: "factory_log",
                chainId: Number(chainId),
                address: address,
