...
import { ZERO_CHECKPOINT_STRING, blockToCheckpoint, encodeCheckpoint, min, } from '../utils/checkpoint.js';
import { bufferAsyncGenerator, createCallbackGenerator, mergeAsyncGenerators, } from '../utils/generators.js';
import { intervalIntersection } from '../utils/interval.js';
...
export async function* getRealtimeEventsOmnichain(params) {
    ...
    let pendingEvents = params.pendingEvents;
    /** Closest-to-tip finalized checkpoint across all chains. */
    let finalizedCheckpoint = ZERO_CHECKPOINT_STRING;
    for await (const { chain, event } of mergeAsyncGeneratorsWithRealtimeOrder(eventGenerators)) {
        const { syncProgress, childAddresses, unfinalizedBlocks } = params.perChainSync.get(chain);
        const eventCallbacks = params.indexingBuild.eventCallbacks[params.indexingBuild.chains.findIndex((c) => c.id === chain.id)];
        ...
        switch (event.type) {
            case "block": {
                ...
                const decodedEvents = decodeEvents(params.common, chain, eventCallbacks, events);
                params.common.logger.trace({
                ...
                const checkpoint = encodeCheckpoint(blockToCheckpoint(event.block, chain.id, "up"));
            ...
            case "reorg": {
                const isReorgedEvent = (_event) => {
                    if (_event.chain.id === chain.id &&
    ...
    for await (const { chain, event } of mergeAsyncGenerators(eventGenerators)) {
        ...
        switch (event.type) {
            ...
            case "finalize": {
                ...
                let finalizeIndex = undefined;
                for (const [index, event] of executedEvents.entries()) {
                    const _chain = params.indexingBuild.chains.find((c) => c.id === event.chain.id);
...
export async function* getRealtimeEventsIsolated(params) {
    if (params.syncProgress.isEnd()) {
        ...
        params.common.metrics.ponder_sync_is_complete.set({ chain: params.chain.name }, 1);
        return;
    }
    const rpc = params.indexingBuild.rpcs[params.indexingBuild.chains.indexOf(params.chain)];
    const eventCallbacks = params.indexingBuild.eventCallbacks[params.indexingBuild.chains.indexOf(params.chain)];
    params.common.metrics.ponder_sync_is_realtime.set({ chain: params.chain.name }, 1);
    const bufferCallback = (bufferSize) => {
        // Note: Only log when the buffer size is greater than 1 because
        // a buffer size of 1 is not backpressure.
        if (bufferSize === 1)
    ...
    const eventGenerator = bufferAsyncGenerator(getRealtimeEventGenerator({
        common: params.common,
        chain: params.chain,
        rpc,
        eventCallbacks,
    ...
    for await (const { chain, event } of eventGenerator) {
        ...
        switch (event.type) {
            case "block": {
                ...
                const events = decodeEvents(params.common, chain, eventCallbacks, rawEvents);
                params.common.logger.trace({
                    msg: "Decoded block events",
                    chain: chain.name,
                    chain_id: chain.id,
...
export async function* getRealtimeEventGenerator(params) {
    ...
    for await (const { block, blockCallback, endClock } of generator) {
        ...
        const syncGenerator = realtimeSync.sync(block, (isAccepted) => {
            ...
            if (isAccepted) {
                params.common.metrics.ponder_realtime_block_arrival_latency.observe({ chain: params.chain.name }, arrivalMs - hexToNumber(block.timestamp) * 1000);
                params.common.metrics.ponder_realtime_latency.observe({ chain: params.chain.name }, endClock());
            }
        ...
        if (block.number === params.syncProgress.end?.number) {
            ...
            params.common.metrics.ponder_sync_is_realtime.set({ chain: params.chain.name }, 0);
            params.common.metrics.ponder_sync_is_complete.set({ chain: params.chain.name }, 1);
            params.common.logger.info({
...
export async function handleRealtimeSyncEvent(event, params) {
    switch (event.type) {
        ...
        case "finalize": {
            ...
            await params.database.syncQB.transaction(async (tx) => {
                ...
                for (const { filter } of params.eventCallbacks) {
                    const completedIntervals = intervalIntersection([finalizedInterval], [
                        [
                            filter.fromBlock ?? 0,
        ...
        case "reorg": {
            ...
            while (params.unfinalizedBlocks.length > 0) {
                const block = params.unfinalizedBlocks[params.unfinalizedBlocks.length - 1];
                if (hexToNumber(block.block.number) > hexToNumber(event.block.number)) {
 ...
 */
export async function* mergeAsyncGeneratorsWithRealtimeOrder(generators) {
    const results = await Promise.all(generators.map((gen) => gen.next()));
    while (results.some((res) => res.done !== true)) {
        let index;
        if (results.some((result) => result.done === false &&
            (result.value.event.type === "reorg" ||
                result.value.event.type === "finalize"))) {
            index = results.findIndex((result) => result.done === false &&
                (result.value.event.type === "reorg" ||
                    result.value.event.type === "finalize"));
        }
        else {
            const blockCheckpoints = results.map((result) => result.done
                ? undefined
                : encodeCheckpoint(blockToCheckpoint(result.value.event.block, result.value.chain.id, "up")));
            const supremum = min(...blockCheckpoints);
            index = blockCheckpoints.findIndex((checkpoint) => checkpoint === supremum);
        }
