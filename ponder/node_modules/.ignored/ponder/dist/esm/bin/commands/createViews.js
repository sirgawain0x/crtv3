...
import { SCHEMATA, createDatabase, getPonderMetaTable, } from '../../database/index.js';
import { getLiveQueryChannelName, getLiveQueryNotifyProcedureName, getViewsLiveQueryNotifyTriggerName, } from '../../drizzle/onchain.js';
...
export async function createViews({ cliOptions, }) {
    ...
    await database.adminQB.wrap((db) => db.execute(sql.raw(`DROP VIEW IF EXISTS "${cliOptions.viewsSchema}"."_ponder_meta"`)));
    await database.adminQB.wrap((db) => db.execute(sql.raw(`DROP VIEW IF EXISTS "${cliOptions.viewsSchema}"."_ponder_checkpoint"`)));
    await database.adminQB.wrap((db) => db.execute(sql.raw(`CREATE VIEW "${cliOptions.viewsSchema}"."_ponder_meta" AS SELECT * FROM "${cliOptions.schema}"."_ponder_meta"`)));
    await database.adminQB.wrap((db) => db.execute(sql.raw(`CREATE VIEW "${cliOptions.viewsSchema}"."_ponder_checkpoint" AS SELECT * FROM "${cliOptions.schema}"."_ponder_checkpoint"`)));
