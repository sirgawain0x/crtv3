...
export const createRealtimeIndexingStore = ({ common, schemaBuild: { schema }, indexingErrorHandler, chainId, }) => {
    ...
    return {
                        ...
                        onConflictDoNothing: storeMethodWrapper(async () => {
                            ...
                            const parseResult = (result) => {
                                ...
                                for (let i = 0; i < values.length; i++) {
                                    if (getCacheKey(table, values[i], primaryKeyCache) ===
                        ...
                        onConflictDoUpdate: storeMethodWrapper(async (valuesU) => {
                            ...
                            if (Array.isArray(values)) {
                                ...
                                for (const value of values) {
                                    ...
                                    if (row) {
                                        const set = typeof valuesU === "function" ? valuesU(row) : valuesU;
                                        validateUpdateSet(table, set, row, primaryKeyCache);
                                        rows.push(await qb.wrap((db) => db
                                            .update(table)
                                            .set(set)
        ...
        delete: storeMethodWrapper(async (table, key) => {
            ...
            const deleted = await qb.wrap((db) => db.delete(table).where(getWhereCondition(table, key)).returning());
