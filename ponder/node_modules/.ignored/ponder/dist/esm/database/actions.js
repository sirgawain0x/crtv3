import { getPartitionName, getPrimaryKeyColumns, getReorgProcedureName, getReorgTableName, getReorgTriggerName, } from '../drizzle/index.js';
import { getColumnCasing, getReorgTable } from '../drizzle/kit/index.js';
import { getLiveQueryChannelName, getLiveQueryNotifyProcedureName, getLiveQueryNotifyTriggerName, getLiveQueryProcedureName, getLiveQueryTriggerName, getViewsLiveQueryNotifyTriggerName, } from '../drizzle/onchain.js';
...
export const dropLiveQueryTriggers = async (qb, { namespaceBuild, tables, chainId, }, context) => {
    await qb.transaction(async (tx) => {
        const notifyTrigger = getLiveQueryNotifyTriggerName();
        await tx.wrap((tx) => tx.execute(`DROP TRIGGER IF EXISTS "${notifyTrigger}" ON "${namespaceBuild.schema}"._ponder_checkpoint;`));
        const trigger = getLiveQueryTriggerName();
        for (const table of tables) {
            const schema = getTableConfig(table).schema ?? "public";
...
export const createViews = async (qb, { tables, views, namespaceBuild, }, context) => {
    await qb.transaction({ label: "create_views" }, async (tx) => {
        ...
        await tx.wrap((tx) => tx.execute(`DROP VIEW IF EXISTS "${namespaceBuild.viewsSchema}"."_ponder_meta"`));
        await tx.wrap((tx) => tx.execute(`DROP VIEW IF EXISTS "${namespaceBuild.viewsSchema}"."_ponder_checkpoint"`));
        await tx.wrap((tx) => tx.execute(`CREATE VIEW "${namespaceBuild.viewsSchema}"."_ponder_meta" AS SELECT * FROM "${namespaceBuild.schema}"."_ponder_meta"`));
        await tx.wrap((tx) => tx.execute(`CREATE VIEW "${namespaceBuild.viewsSchema}"."_ponder_checkpoint" AS SELECT * FROM "${namespaceBuild.schema}"."_ponder_checkpoint"`));
        const notifyProcedure = getLiveQueryNotifyProcedureName();
