...
import { SHARED_OPERATION_ID_SEQUENCE, sqlToReorgTableName, } from '../drizzle/kit/index.js';
import { getLiveQueryNotifyProcedureName, getLiveQueryProcedureName, } from '../drizzle/onchain.js';
...
CREATE TABLE IF NOT EXISTS "${namespace.schema}"."_ponder_checkpoint" (
  ...
  "finalized_checkpoint" VARCHAR(75) NOT NULL
                ...
                await tx.wrap((tx) => tx.execute(`CREATE SEQUENCE IF NOT EXISTS "${namespace.schema}"."${SHARED_OPERATION_ID_SEQUENCE}" AS integer INCREMENT BY 1`), context);
            };
            const tryAcquireLockAndMigrate = () => adminQB.transaction({ label: "migrate" }, async (tx) => {
                ...
                if (previousApp.is_dev === 1) {
                    ...
                    for (const table of previousApp.table_names ?? []) {
                        ...
                        await tx.wrap((tx) => tx.execute(`DROP TABLE IF EXISTS "${namespace.schema}"."${sqlToReorgTableName(table)}" CASCADE`), context);
                    }
                    ...
                    endClock = startClock();
                    await tx.wrap((tx) => tx.execute(`DROP TABLE IF EXISTS "${namespace.schema}"."${getTableName(PONDER_CHECKPOINT)}" CASCADE`), context);
                    await tx.wrap((tx) => tx.execute(`DROP TABLE IF EXISTS "${namespace.schema}"."${getTableName(PONDER_META)}" CASCADE`), context);
