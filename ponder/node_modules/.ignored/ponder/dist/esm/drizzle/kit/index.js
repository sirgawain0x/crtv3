    ...
    if (schema && schema !== "public") {
        return pgSchema(schema).table(sqlToReorgTableName(getTableName(table)), {
            operation_id: integer()
                .notNull()
                .primaryKey()
                .default(sql.raw(`nextval('"${schema}"."${SHARED_OPERATION_ID_SEQUENCE}"')`)),
    ...
    const reorgColumns = Object.values(squashPgScheme(generatePgSnapshot([
        pgTable("", {
            operation_id: integer()
                .notNull()
                .primaryKey()
                .default(sql.raw(`nextval('"${statement.schema === "" ? "public" : statement.schema}"."${SHARED_OPERATION_ID_SEQUENCE}"')`)),
