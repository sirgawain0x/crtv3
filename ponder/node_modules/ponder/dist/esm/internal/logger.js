...
export function createLogger({ level, mode = "pretty", }) {
    ...
    const _createLogger = (logger) => {
        return {
            error(options, printKeys) {
...
const format = (log) => {
    ...
    if (pc.isColorSupported) {
        ...
        if (PRINT_KEYS in log) {
            for (const key of log[PRINT_KEYS]) {
                keyText += ` ${key}=${log[key]}`;
            }
        }
        else {
