...
import { NonRetryableUserError, nonRetryableUserErrorNames, } from '../internal/errors.js';
...
export async function isolatedController({ common, preBuild, namespaceBuild, schemaBuild, indexingBuild, crashRecoveryCheckpoint, database, }) {
    const backfillEndClock = startClock();
    const perChainState = new Map();
    ...
    const callback = async () => {
        if (isAllReady === false &&
            indexingBuild.chains.every((chain) => perChainState.get(chain.id) !== "backfill")) {
            isAllReady = true;
            common.logger.info({
        ...
        if (isAllComplete === false &&
            indexingBuild.chains.every((chain) => perChainState.get(chain.id) === "complete")) {
            isAllComplete = true;
        }
    };
    if (common.options.command === "dev" ||
        ...
        common.options.indexingCacheMaxBytes = Math.floor(common.options.indexingCacheMaxBytes / indexingBuild.chains.length);
        common.options.rpcMaxConcurrency = Math.floor(common.options.rpcMaxConcurrency / indexingBuild.chains.length);
        common.options.syncEventsQuerySize = Math.floor(common.options.syncEventsQuerySize / indexingBuild.chains.length);
        await Promise.all(indexingBuild.chains.map(async (chain) => {
            const chainIndex = indexingBuild.chains.findIndex((c) => c.id === chain.id);
        ...
        const perThreadChains = chunk(indexingBuild.chains, Math.ceil(indexingBuild.chains.length / common.options.maxThreads));
        const perThreadWorkers = [];
        for (const chains of perThreadChains) {
            ...
            const perThreadHeapSizeLimit = Math.floor(heapSizeLimit / (common.options.maxThreads + 1) / 1024 / 1024);
        ...
        common.metrics = new AggregateMetricsService(common.metrics, perThreadWorkers);
        common.shutdown.add(() => {
...
//# sourceMappingURL=isolatedController.js.map