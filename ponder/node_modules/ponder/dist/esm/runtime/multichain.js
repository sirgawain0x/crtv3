            ...
            case "block": {
                ...
                await database.userQB.transaction(async (tx) => {
                    if (database.userQB.$dialect === "postgres") {
                        await tx.wrap((tx) => tx.execute("CREATE TEMP TABLE live_query_tables (table_name TEXT PRIMARY KEY) ON COMMIT DROP"), context);
                    }
                    else {
                        await tx.wrap((tx) => tx.execute("CREATE TEMP TABLE IF NOT EXISTS live_query_tables (table_name TEXT PRIMARY KEY)"), context);
                    }
                    ...
                    if (database.userQB.$dialect === "pglite") {
                        await tx.wrap((tx) => tx.execute("TRUNCATE TABLE live_query_tables"), context);
                    }
                }, undefined, context);
