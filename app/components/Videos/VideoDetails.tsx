...
export default function VideoDetails(props: VideoDetailsProps) {
  ...
  useEffect(() => {
    async function fetchAsset() {
      try {
        const assetResponse = await getDetailPlaybackSource(props?.asset?.id);
  ...
  const src = getSrc(props?.asset?.playbackId);

  return (
    <main>
      <h1 className={'p-4'}>Video Detail Page</h1>
      <div className={'p-4'}>Asset ID: {props?.asset.id}</div>
      {isLoading ? (
        ...
        <div className="container max-w-md">
          <h2 className={'size-md my-4'}>{props?.asset?.name}</h2>
          {props?.asset?.id && (
            <Player.Root src={src}>
