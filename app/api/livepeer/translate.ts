...
export const getLivepeerTranslation = async (params: TranslateParams) => {
  try {
    ...
    const livepeerApiUrl =
      process.env.LIVEPEER_API_URL || 'https://dream-gateway.livepeer.cloud';

    const formData = new FormData();

    formData.append(
      'prompt',
      `Translate '${params.text}' from ${params.source} to ${params.target}. Do not include any other words than the exact, grammatically correct translation.`,
    );
    ...
    if (!result.ok) {
      throw new Error(
        `Livepeer API error ${result?.status}: ${result?.statusText}`,
      );
    }
