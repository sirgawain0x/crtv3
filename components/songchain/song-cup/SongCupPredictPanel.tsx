...
import {
  ...
  SongCupPredictFeedPlaceholder,
  SongCupPredictFilterTabs,
  SongCupPredictGradientGlow,
  SongCupPredictPortfolioPill,
  SongCupPredictTitle,
  type SongCupPredictTab,
...
export function SongCupPredictPanel({ className }: SongCupPredictPanelProps) {
  ...
  const [activeTab, setActiveTab] = useState<SongCupPredictTab>("games");
    ...
    <SongCupPredictContentWell className={className}>
      ...
      <div className="relative z-10 flex flex-col gap-6">
        <div className="flex items-start justify-between gap-4">
          ...
          <SongCupPredictPortfolioPill className="shrink-0" />
        </div>

        {event ? <SongCupPredictEventHeader event={event} /> : null}

        <SongCupPredictFilterTabs active={activeTab} onChange={setActiveTab} />
