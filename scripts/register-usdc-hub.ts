 ...
 *
 * Prerequisites (meTokens-core repo):
 *   1. Deploy a USDC-compatible vault (or reuse pattern from Hub 1)
 *   2. yarn hardhat add-vault --diamond <Diamond> --registry <VaultRegistry> --vault <Vault> --network base
 *   3. Run this script with DIAMOND_OWNER_PRIVATE_KEY set
 *
 * Hub 1 (DAI) on Base for reference:
 *   asset:  0x50c5725949A6F0c72E6C4a641F24049A917DB0Cb
 *   vault:  0xff6Eb470bf0D817B17DFD596F1B2b3110682a40f
 *   refundRatio / baseY / reserveWeight: see DEFAULT_HUB_CURVE_PARAMS
 *
 * Usage:
 *   USDC_VAULT_ADDRESS=0x... DIAMOND_OWNER_PRIVATE_KEY=0x... npx tsx scripts/register-usdc-hub.ts
 */
...
USDC on Base: ${USDC}
Diamond:      ${DIAMOND}
Target hub:   ${USDC_HUB_ID} (next available after DAI hub 1)
`);
    process.exit(1);
  }
