 ...
 *
 * Usage:
 *   HUB_SYMBOL=USDS VAULT_ADDRESS=0x... DIAMOND_OWNER_PRIVATE_KEY=0x... npx tsx scripts/register-stable-hub.ts
 *
 * Supported HUB_SYMBOL values: USDS, USDC, GHO
 *
 * Hub IDs (planned):
 *   1 — DAI (legacy, already live)
 *   2 — USDS
 *   3 — USDC (recommended)
 *   4 — GHO
 */
...
Required:
  ...
  VAULT_ADDRESS              Deployed vault for this asset (meTokens-core add-vault)
  DIAMOND_OWNER_PRIVATE_KEY  Admin wallet

Optional:
  HUB_OWNER_ADDRESS          Defaults to signer

Configured assets:
${REGISTERABLE.map((s) => `  Hub ${HUB_ASSET_CONFIGS[s].hubId} — ${s} @ ${HUB_ASSET_CONFIGS[s].address}`).join('\n')}
