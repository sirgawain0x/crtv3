...
class CompositeMerge extends Command {
    async run() {
        const parsed = await this.parse(CompositeMerge);
        const compositePaths = parsed.raw.filter((token)=>{
        ...
        try {
            ...
            const composites = await Promise.all(compositePaths.map(async (path)=>await readEncodedComposite(this.ceramic, path)));
            const commonEmbedsFlag = this.flags['common-embeds'];
            ...
            if (commonEmbedsFlag) {
                compositeOptions = {
                    commonEmbeds: [
                        'all',
                        'none'
                    ].includes(commonEmbedsFlag) ? commonEmbedsFlag : commonEmbedsFlag.split(',')
                };
            }
            ...
            if (this.flags.output != null) {
                ...
                this.spinner.succeed(`Composite was created and its encoded representation was saved in ${output}`);
            } else {
...
_define_property(CompositeMerge, "strict", false);
_define_property(CompositeMerge, "description", 'create a Composite from other composites');
_define_property(CompositeMerge, "flags", {
    ...Command.flags,
...
export { CompositeMerge as default };
