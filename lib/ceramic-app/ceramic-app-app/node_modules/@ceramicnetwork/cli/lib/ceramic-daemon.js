...
import { EnvironmentUtils, LoggerProvider, LogStyle, Networks, StreamUtils, SyncOptions, UnreachableCaseError, } from '@ceramicnetwork/common';
...
export class CeramicDaemon {
    ...
    static async create(opts) {
        ...
        try {
            ...
            throw new Error(`Error connecting to p2p node. Make sure ceramic-one is running or use --ipfs-api to modify the expected http location (currently ${opts.ipfs?.host}): ${err}`);
        }
        const desiredIpfsClient = EnvironmentUtils.useRustCeramic() ? 'ceramic-one' : 'kubo';
        const wrongIpfsClient = opts.ipfs.mode == IpfsMode.REMOTE &&
        ...
        diagnosticsLogger.imp(`Connecting to IPFS node with version "${ipfsId.agentVersion}" available as ${ipfsId.addresses
            .map(String)
            .join(', ')}`);
        if (wrongIpfsClient) {
            if (desiredIpfsClient === 'ceramic-one') {
                diagnosticsLogger.warn(`Detected ipfs kubo running at ${ipfsId.addresses} with agent version ${ipfsId.agentVersion}. Using kubo for p2p networking is deprecated - we recommend migrating to ceramic-one instead.`);
            }
            else {
                diagnosticsLogger.warn(`Ignoring configuration IPFS_FLAVOR='go'. Detected remote ceramic-one running at ${ipfsId.addresses} with agent version ${ipfsId.agentVersion}. If you intended to use ipfs, specify an --ipfs-url for an ipfs node.`);
            }
        }
        ...
        if (opts.stateStore?.mode == StateStoreMode.S3) {
            const s3kvFactory = new S3KVFactory(opts.stateStore?.s3Bucket, params.networkOptions.name, diagnosticsLogger, opts.stateStore?.s3Endpoint);
