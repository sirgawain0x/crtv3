...
import { EnvironmentUtils } from '@ceramicnetwork/common';
    ...
    static async buildIpfsConnection(mode, network, logger, ipfsEndpoint) {
        if (mode == IpfsMode.REMOTE) {
            ...
            if (EnvironmentUtils.useRustCeramic()) {
                throw new Error(`Running ceramic-one in bundled mode is not supported. Pass --ipfs-api to connect to an external ceramic-one node.`);
            }
            else {
